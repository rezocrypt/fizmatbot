...
from Files.Weather.weather import getweather
...
19. 🌡 /weather-- Եղանակ
20. 🧑‍💻 /programmer -- Ծրագրային Մշակող 
...
# Opening units file text and reading it
def getunits():
...
async def cmd_start(message: types.Message):
    ...
    button_18 = types.KeyboardButton(text="🌡Եղանակ🌡")
    keyboard.add(button_18)
    button_19 = types.KeyboardButton(text="🧑‍💻Ծրագրային Մշակող🧑‍💻")
...
async def cmd_start(msg: types.Message):
    ...
    #    await bot.send_photo(chat_id=msg.from_user.id, photo=open(f'Photos/PhysmathToday/{)

    for i in range(1,8):
        media.attach_photo(types.InputFile(f'Photos/PhysmathToday/{i}.jpg'))
        text += f"🖼 {i}.{captions.alist[i-1]}\n\n"
    media.attach_photo(types.InputFile('Photos/PhysmathToday/1.jpg'), 'Cat!')
    media.attach_photo(types.InputFile('Photos/PhysmathToday/2.jpg'), 'Cat!')
...
@dp.message_handler(lambda message: message.text == "🌡Եղանակ🌡")
async def cmd_start(msg: types.Message):
    addLog(msg,"Եղանակ")
    weather = getweather()
